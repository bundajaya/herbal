    ...
    function renderFotoPreviews() {
        ...
        c.innerHTML = uploadedFotos.map((url, i) => `
            <div class="foto-thumb">
                ...
                <button type="button" class="del-btn" onclick="hapusFotoPreview(${i})">×</button>
            </div>`).join('');
    }
            ...
            c.innerHTML = Object.entries(data).map(([id, p]) => {
                ...
                <div class="produk-admin-card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="text-xs text-gray-400 mb-3"><i class="fas fa-fire mr-1 text-orange-400"></i>${p.terjual||0} terjual · ⭐${p.avgRating||'–'} (${p.ulasanCount||0})</p>
                        <div class="flex gap-2">
    ...
    function editProduk(id) {
        db.ref('produk/' + id).once('value').then(snap => {
            const p = snap.val();
            if (!p) return;
            document.getElementById('produkModalTitle').textContent = '✏️ Edit Produk';
            resetProdukForm();
    ...
    function simpanProduk(e) {
        e.preventDefault();
        if (isUploadingFoto) {
            return showToast('⏳ Tunggu upload foto selesai dulu', 'error');
        }
        ...
        // update() dengan value null akan menghapus key tsb di Firebase RTDB —
